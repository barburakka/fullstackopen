...
blogRouter.put('/:id', async (request, response) => {
  const { title, url, author, likes, user, comments } = request.body
  const updatedPost = await Blog.findByIdAndUpdate(request.params.id,  { title, url, author, likes, user, comments }, { new: true } )
...
blogRouter.post('/:id/comments', async (request, response) => {
  ...
  const { title, url, author, likes, user, comments } = await Blog.findById(request.params.id)
  const updatedPost = await Blog.findByIdAndUpdate(request.params.id,  { title, url, author, likes, user, comments: [...comments, comment] }, { new: true } )
  await updatedPost.populate('user', { username: 1, name: 1 })
  response.json(updatedPost)
})
